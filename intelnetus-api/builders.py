...
class PublicationBuilder:
    def __init__(self, publication_data, year, doi):
        ...
        self.abstract = get_sql_syntax(
            remove_common_words(
                PublicationBuilder.get_abstract(
        ...
        self.keywords = get_sql_syntax(
            PublicationBuilder.get_keywords(
                get_safe_attribute(publication_data, 'authkeywords', 'string')
            )
        )
        
        self.fields = get_sql_syntax(
            PublicationBuilder.get_fields(
                get_safe_attribute(publication_data, 'subject_areas', 'string')
            )
        )
        
        self.fields_abbreviations = get_sql_syntax(
            PublicationBuilder.get_fields_abbreviations(
                get_safe_attribute(publication_data, 'subject_areas', 'string')
            )
        )
        
        self.citations_count = PublicationBuilder.get_maximum_citations_count(
            get_safe_attribute(publication_data, 'citedby_count', 'number'),
            doi
        )
        
        self.authors_number = PublicationBuilder.get_authors_number(
            get_safe_attribute(publication_data, 'authors', 'list')
        )
        
        self.affiliations_number = PublicationBuilder.get_affiliations_number(
...
class AuthorBuilder:
    def __init__(self,author_data):
        ...
        self.fields_of_study = get_sql_syntax(
            AuthorBuilder.get_fields(get_safe_attribute(author_data, 'subject_areas', 'string'))
        )
        
        self.citations_count = get_safe_attribute(author_data, 'cited_by_count', 'number')
        self.affiliations = get_sql_syntax(
            AuthorBuilder.get_affiliations(get_safe_attribute(author_data, 'affiliation_history', 'string'))
        )
...
class OrganizationBuilder:
    def __init__(self, organization_data):
        ...
        self.type_1, self.type_2 = OrganizationBuilder.get_affiliation_types(organization_data)
